...
# KL loss weight (start small)
LAMBDA_RAT = 0.05
EPS = 1e-8

JSON_FILE = "contriever_train_triplets_ev_offset.jsonl_dedup.jsonl"
SAVE_DIR = "contriever-finetuned-triplet+kl_evmask60"
...
# -----------------------------
# Triplet 1-N dataset with rationale mask (now: evidence mask on the positive)
# -----------------------------
class TripletDataset1N(Dataset):
    def __init__(self, json_file, tokenizer, max_length=MAX_LENGTH):
        ...
        with open(json_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # Evidence fields are required in the JSONL
                if (
                    query and pos and neg_list and len(neg_list) > 0
                    and ev_s is not None and ev_e is not None
                    and int(ev_e) > int(ev_s) >= 0
                ):
                    self.data.append({
                        "query": query,
                        "positive": pos,
    ...
    def __getitem__(self, idx):
        ...
        # Offset mapping is required to build the evidence-based mask
        ...
        # -----------------------------
        # Rationale mask: exact evidence mask
        # - uses token offsets over the positive chunk text
        # - marks 1 if the token intersects [evidence_start, evidence_end)
        # - zeros CLS and special tokens (offset (0, 0))
        # - safe fallback: if evidence disappears due to truncation, revert to the full-positive mask
        # -----------------------------
        ...
        for i in range(attn.size(0)):
            ...
            if ts == 0 and te == 0:
                continue
            if not (te <= ev_s or ts >= ev_e):
                rat_mask[i] = 1.0

        # Explicitly remove CLS (position 0), even if offsets are not (0, 0)
        rat_mask[0] = 0.0

        # Fallback: if nothing was marked (e.g., evidence was truncated), use the full-positive mask without CLS
...
def rationale_kl_loss(cls_to_tok, rat_mask, pos_attn, eps=1e-8):
    ...
    rat_mask:   [B, T] binary evidence mask with CLS set to 0
    pos_attn:   [B, T] positive attention mask (1 for non-PAD, 0 for PAD)
    """
...
for epoch in range(EPOCHS):
    ...
    for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
        ...
        # KL rationale loss (positives only)
